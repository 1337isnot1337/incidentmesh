...
import { DeveloperMessageItem, FunctionCallItem, FunctionCallOutputItem, ModelMessageItem, ReasoningItem, SemanticEvent, SystemMessageItem, UserMessageItem, } from "@mozaik-ai/core";
 ...
 */
export class GeminiSignaturePreservingRunner {
    signatures = new Map();
    callNames = new Map();
    client;
    constructor(client) {
        this.client = client ?? new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
    }
    async run(request) {
        ...
        for (const item of request.context.getItems()) {
            if (item instanceof DeveloperMessageItem || item instanceof SystemMessageItem) {
                ...
                if (item.content?.text)
                    add("model", { text: item.content.text, thought: true });
            }
        ...
        for (const part of parts) {
            if (part.functionCall) {
                ...
                if (part.thoughtSignature)
                    this.signatures.set(callId, part.thoughtSignature);
                ...
                items.push(ReasoningItem.rehydrate({ content: { type: "input_text", text: part.text }, encryptedContent: undefined, summary: [] }));
            }
            else if (part.text) {
