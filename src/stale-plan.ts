...
function normalize(hypotheses: readonly Hypothesis[]): FrozenHypothesis[] {
  return hypotheses
    .map(({ role, claim, confidence, rootCause }) => ({ role, claim, confidence, rootCause }))
    .sort((a, b) => ROLES.indexOf(a.role) - ROLES.indexOf(b.role))
}

function armFromReport(schedule: "concurrent" | "sequential", report: IncidentReport): StalePlanArm {
  const plan = report.action.plans[0]
  const attempt = report.action.attempts[0]
  if (plan === undefined || attempt === undefined) throw new Error(`${schedule} runtime did not produce a plan and action attempt`)
  const replanAttempt = report.action.attempts[1]
  const targetCause = plan.hypotheses.find((item) => item.role === "trace")?.rootCause
  if (targetCause === undefined) throw new Error(`${schedule} plan did not freeze trace evidence`)
  const peerEvidenceDuringPlanning = report.hypotheses.some((item) =>
    item.role !== "trace" && item.atMs >= plan.startedAtMs && item.atMs <= attempt.atMs)
  return {
    schedule,
...
export async function runStalePlanAblation(input: {
  ...
  const run = (scheduleMode: "concurrent" | "sequential") => runIncidentScenario({
    dryRun: true,
